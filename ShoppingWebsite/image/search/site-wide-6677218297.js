...
var IAC=function(sb,pe,iac){var sbPlaceHolder,sbPlaceHolderDiv,sbNode,sbDiv,iacNode,iacDiv,canShowIAC=true,iacType=0;
function get(){return iacNode||create();
}function create(){var p=sb.pos(true),d=sb.size(true);
sbPlaceHolder=$("<input id='sbPlaceHolder' class='searchSelect' readOnly='true'/>").css({top:p.top,left:p.left,width:"100%",border:"2px inset"}).appendTo(pe||sb.parent());
...
sbDiv=sbNode.get(0);
iacNode=$("<input id='inline_auto_complete' class='searchSelect' readOnly='true'/>").css({left:p.left,top:p.top,"z-index":1,color:"#999",position:"absolute","background-color":"#FFF",width:d.width}).appendTo(pe||sb.parent());
iacDiv=iacNode.get(0);
setInterval(adjust,200);
sbNode.keydown(keyDown);
return iacNode;
}function adjust(){var p=sb.pos(),d=sb.size();
sbNode.css({left:p.left,top:p.top,width:d.width});
iacNode.css({left:p.left,top:p.top,width:d.width});
...
lookup(opt,"sb");
if(!opt.sb){return;
}searchBox=new SB(opt.sb,{adjust:move,hide:hideSuggestions,dismiss:dismissSuggestions,change:(staticContent?update:delayUpdate),submitEnabled:submitEnabled,keyPress:keyPress,click:clickHandler,newDesign:newDesign,iac:opt.iac,checkIAC:checkIAC});
lookup(opt,"pe");
if(opt.iac){inlineAutoComplete=new IAC(searchBox,opt.pe,opt.iac);
}searchSuggest=new SS(searchBox,opt.pe,opt.iac);
lookup(opt,"form");
lookup(opt,"valInput");
lookup(opt,"dd");
lookup(opt,"submit");
